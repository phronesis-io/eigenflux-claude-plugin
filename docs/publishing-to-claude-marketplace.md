...
This guide documents the two paths for distributing this repository's Claude Code plugin to Claude Code users:
...
Our `marketplace.json` uses an explicit `url` source pointing at this repo's default branch — this matches the dominant pattern in `claude-plugins-official` (51% of listed plugins use `{"source": "url", "url": "..."}`):

```json
{
  "name": "eigenflux-marketplace",
  "owner": {
    "name": "EigenFlux",
    "email": "contact@eigenflux.one"
  },
  "metadata": {
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "url",
        "url": "https://github.com/phronesis-io/eigenflux-claude-plugin.git"
      },
      "description": "Feed polling, PM streaming, and broadcast network skills for Claude Code.",
      "homepage": "https://www.eigenflux.ai",
      "repository": "https://github.com/phronesis-io/eigenflux-claude-plugin",
      "license": "MIT",
...
Options, ordered by frequency in `claude-plugins-official` (snapshot of 140 plugins, April 2026):

| Source | Use when | Count in official |
|---|---|---|
| `{ "source": "url", "url": "https://github.com/owner/repo.git" }` | Plugin lives at the root of its own repo (our case). Omit `sha`/`ref` to always follow the default branch. | 72 |
| `"./<path>"` (relative) | The marketplace and the plugin live in the same repo — Anthropic uses this for plugins they host themselves. Only works for git-based installs. | 49 |
| `{ "source": "git-subdir", "url": "...", "path": "plugins/xxx", "ref": "main" }` | Plugin lives in a subdirectory of a larger monorepo — sparse clone. | 18 |
| `{ "source": "github", "repo": "owner/repo" }` | Shorthand for a whole-repo GitHub plugin. Rarely used in official (1 of 140). | 1 |
| `{ "source": "npm", "package": "@scope/pkg" }` | You publish the plugin to npm instead of a git repo. | 0 |

Pinning: add `"sha": "<40-char commit>"` to lock a specific commit (47 of 72 `url` entries do this). Our entry intentionally leaves it out so `claude-plugins-official` auto-update pulls our latest `main` — but that means `main` has to be green all the time.
...
claude plugin install eigenflux@eigenflux-marketplace
```
...
Additionally run the end-to-end harness: `node tests/e2e-test.mjs` (spawns a child `claude -p`, verifies slash command, skills, MCP tools, SessionStart hook). See `docs/e2e-test-report.md`.
...
1. Bump `version` in `.claude-plugin/plugin.json` (or run `pnpm bump-version <version>`).
2. Update `CHANGELOG.md`.
3. Commit + tag.
4. Push.
5. Users get the update automatically (marketplace auto-update runs at session start — off by default for third-party marketplaces; users can toggle it on via `/plugin` → Marketplaces → *this marketplace* → Enable auto-update) or manually via `/plugin marketplace update eigenflux-marketplace`.
...
```json
{
  "extraKnownMarketplaces": {
    "eigenflux-marketplace": {
      "source": { "source": "github", "repo": "phronesis-io/eigenflux-claude-plugin" }
    }
  },
  "enabledPlugins": {
    "eigenflux@eigenflux-marketplace": true
  }
}
```
...
- [ ] Runs the e2e harness (`node tests/e2e-test.mjs`) green on a clean machine.
...
1. `pnpm build` — clean `dist/` reflects source.
2. Bump `version` in `.claude-plugin/plugin.json` (or `pnpm bump-version <version>`).
3. Update `CHANGELOG.md`.
4. `claude plugin validate .` — zero errors.
5. `node tests/e2e-test.mjs` — all assertions pass (`docs/e2e-test-report.md`).
