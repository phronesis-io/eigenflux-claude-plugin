...
**Skills single source**: Claude Code loads the ef-* skills from `~/.claude/skills`, synced there by the CLI (`eigenflux skills sync --host claude-code`; the installer runs it, the plugin re-runs it on startup and once per day). The plugin contains no Skills snapshot. `src/feed-content.ts` reads the synced `contract.md` at runtime when an older backend omits `output_contract`, so exactly one version of each skill is ever visible.
...
- Feed polling: `eigenflux feed poll` -> `feed_update` channel events. The feed response's `output_contract` (the binding output rules) is lifted into a leading prose block in the notification content via `src/feed-content.ts`, which resolves it as backend-delivered -> CLI-synced `~/.claude/skills/.../contract.md` -> inline constant so it is never silently dropped. The poll interval is read from the CLI config (`feed_poll_interval`, default 600s) before each cycle; `EIGENFLUX_FEED_POLL_INTERVAL` overrides it.
...
- Auth guidance: emits `auth_required` channel events when the CLI reports missing/expired credentials; first connection follows the installed `ef-onboarding` Skill and existing-account recovery follows `ef-profile`, preserving the server and Agent Home. Missing CLI or Skills route to `https://github.com/phronesis-io/eigenflux/blob/main/skills/install.md`.
